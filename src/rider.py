...
class Rider(Entity):
    ...
    def move_rider(self, deck, backward=False):
        ...
        # No caso não-tão-raro de vetores (0, y), move o jogador de acordo
            ...
            and self.__temp_player_center[1] + 2 < self.__temp_player_target[1]
        ):
            self.__change_move(self.clicked_card, self.__timer)
    ...
    def __set_temp_variables(self):
        ...
        if self.clicked_card[0] < 0:
            self.__temp_player_center = (
                -self.__temp_player_center[0],
                self.__temp_player_center[1],
            )
            self.__temp_player_target = (
                -self.__temp_player_target[0],
                self.__temp_player_target[1],
            )
    ...
    def select_card(self, card):
        ...
        self.__player_target = (
            card[0] * DISTANCE + self.__player_target[0],
            -card[1] * DISTANCE + self.__player_target[1],
        )
    ...
    def update_death(self):
        # Quando acabar os eventos de clock, apaga a imagem do rider
    ...
    def __get_last_vector(self):
        ...
        card = (
            self.rect.center[0] - self._path[-1][0],
            self.rect.center[1] - self._path[-1][1],
        )
...
class Player:
    ...
    def restart(self, deck):
        # Remove o antigo e inicia um novo
        if self.__wrapper.sprite:
            self.__wrapper.sprite.kill()

        self.__init__(1, (GRID_X / 2 - 1, GRID_Y / 2 - 2), (RIDER_X, RIDER_Y), deck)
