...
class Menu(Entity):
    ...
    def __preview_selected_button(button, screen):
        ...
        rect_pos = (
            button.rect.left - BUTTON_SELECTED_WIDTH,
            button.rect.top - BUTTON_SELECTED_WIDTH,
        )
        rect_size = (
            button.rect.width + 2 * BUTTON_SELECTED_WIDTH,
...
class MainMenu(Menu):
    ...
    def __init__(self, game, image_path, x_y, scale_size):
        ...
        self.buttons_group = pygame.sprite.Group(
            self.btn_the_grid, self.btn_options, self.btn_credits
        )
...
class OptionsMenu(Menu):
    ...
    def __init__(self, game, image_path, x_y, scale_size):
        ...
        self.vol_position = (
            (self.vol_x + 2 * self.vol_space, self.vol_y),
            (self.vol_x + 3 * self.vol_space, self.vol_y),
            (self.vol_x + 4 * self.vol_space, self.vol_y),
            (self.vol_x + 5 * self.vol_space, self.vol_y),
            (self.vol_x + 6 * self.vol_space, self.vol_y),
        )
        self.vol_dict = {
            "vol_1": 0,
            "vol_2": 0.25,
        ...
        self.volume_image = Entity(
            TEXTURE_MENU_PATH + "volume_text.png",
            (self.vol_x - 30, self.vol_y),
            (BUTTON_X, BUTTON_Y),
        )
        self.volume_image.rect = self.volume_image.image.get_rect(
            center=(self.vol_x - 30, self.vol_y)
        )
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.state_control.screen.blit(
                self.volume_image.image, self.volume_image.rect
            )
    ...
    def change_volume(self, vol):
        ...
        temp_vol = int(self.state_control.volume / 0.15)
        vol = int(vol[-1])
        ...
        self.state_control.volume = 0.15 * (vol - 1)
...
class CreditsMenu(Menu):
    ...
    def __init__(self, game, image_path, x_y, scale_size):
        ...
        self.background_image = Entity(
            TEXTURE_MENU_PATH + "background_credits.png", (0, 0), (1000, 750)
        )
    ...
    def display_menu(self):
        ...
        while self.run_display:
            ...
            self.draw_text(
                ...
                self.font_size[1],
                self.txt_x,
                (self.txt_y + 2 * self.space_size[0]),
            )
            self.draw_text(
                "Beatriz Miranda Bezerra",
                self.font_size[0],
                self.txt_x,
                (self.txt_y + 3 * self.space_size[0]),
            )
            self.draw_text(
                "Gustavo Murilo Cavalcante Carvalho",
                self.font_size[0],
                self.txt_x,
                (self.txt_y + 4 * self.space_size[0]),
            )
            self.draw_text(
                "Henzo Felipe Carvalho de Mattos",
                self.font_size[0],
                self.txt_x,
                (self.txt_y + 5 * self.space_size[0]),
            )
            self.draw_text(
                "- Art and Concept granted by -",
                self.font_size[1],
                self.txt_x,
                (self.txt_y + 7 * self.space_size[0]),
            )
            self.draw_text(
                "Tulio Konecny",
                self.font_size[0],
                self.txt_x,
                (self.txt_y + 8 * self.space_size[0]),
            )
...
class TutorialScreen(Menu):
    ...
    def __init__(self, game, image_path, x_y, scale_size):
        ...
        self.img_wall = Entity(
            IMG_MANUAL_PATH + "collision_with_side_walls.png",
            (self.img_x, self.img_y),
            (200, 200),
        )
        self.img_moto = Entity(
            IMG_MANUAL_PATH + "intersection_with_motorcycle.png",
            (2 * self.img_x + self.space_img, self.img_y),
            (200, 200),
        )
        self.img_line = Entity(
            IMG_MANUAL_PATH + "intersection_with_the_line.png",
            (3 * self.img_x + 2 * self.space_img, self.img_y),
            (200, 200),
        )

        # Define os botões dessa tela
        self.btn_play = Button(
            TEXTURE_MENU_PATH + "play_button.png",
            (WIDTH / 2, HEIGHT - 80),
            (BUTTON_X, BUTTON_Y),
            "play",
        )
        ...
        while self.run_display:
            ...
            if self.page == 1:
                ...
                self.draw_large_text(
                    "Colisões com paredes", self.font_size, 250, 300, 15
                )
                self.draw_large_text("Colisões diretas", self.font_size, 500, 300, 15)
                self.draw_large_text(
                    "Colisões com as linhas", self.font_size, 750, 300, 15
                )
