...
class GridGame(Entity):
    ...
    def __init__(self, image_path, x_y, scale_size, bot_number, volume, state_control):
        ...
        # Cria o jogador
        self._player = Player(
            1, (GRID_X / 2 - 1, GRID_Y / 2 - 2), (RIDER_X, RIDER_Y), self._deck
        )
        ...
        self._all_riders = pygame.sprite.Group(
            (self._player.sprite()), self._bots.sprites()[::-1]
        )
    ...
    def draw(self, screen):
        ...
        for rider in self._all_riders.sprites():
            pygame.draw.lines(
                screen, rider._color, False, rider._path + [rider.rect.center], width=6
            )
    ...
    def __preview_selected_card(card, screen):
        ...
        rect_pos = (
            card.rect.left - CARD_SELECTED_WIDTH,
            card.rect.top - CARD_SELECTED_WIDTH,
        )
        rect_size = (
            card.rect.width + 2 * CARD_SELECTED_WIDTH,
    ...
    def __preview_selected_path(self, card, screen):
        ...
        size_segments = (
            12  # Defina a distância desejada entre os segmentos da linha tracejada
        )
        num_segments = int(
            start.distance_to(start + card_value * DISTANCE) / size_segments
        )
        ...
        for i in range(0, num_segments, 2):
            ...
            pygame.draw.line(
                screen, self._player._color, segment_start, segment_end, width=5
            )
    ...
    def __first_turn_collision(self):
        ...
        for rider in self._all_riders.sprites()[::-1]:
            ...
            if (
                utilities.check_line_collision(self._all_riders, rider)
                and self._game_turn
            ):
                rider.kill_rider()
                self.sound[0].play()
