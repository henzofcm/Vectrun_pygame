...
class Grid_Game(Entity):
    ...
    def __preview_selected_path(self, card, screen):
        ...
        start = pygame.Vector2(self._player.sprite._path[-1])  # Converte para Vector2
        card_value = pygame.Vector2(card.value[0], -card.value[1])
        size_segments = 5  # Defina a distância desejada entre os segmentos da linha tracejada
        ...
        for i in range(num_segments):
            if i % 2 == 0:
                ...
                pygame.draw.line(screen, self._player.sprite._color, segment_start, segment_end, width=4)     
